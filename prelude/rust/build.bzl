...
def generate_rustdoc(
    ...
    if url_prefix != None:
        ...
        for dep in resolve_rust_deps(ctx):
            ...
            rustdoc_cmd.add(
                "--extern-html-root-url={}={}/{}:{}".format(
                    name, url_prefix, dep.label.package, dep.label.name
                ),
            )
...
def generate_rustdoc_test(
    ...
    rustdoc_cmd.hidden(
        compile_ctx.symlinked_srcs, link_args_output.hidden, runtime_files
    )
...
def rust_compile(
    ...
    # print(compile_ctx.cxx_toolchain_info.linker_info.linker)
    toolchain_info = compile_ctx.toolchain_info
    ...
    rustc_cmd = cmd_args(
        ...
        ["--json=unused-externs-silent", "-Wunused-crate-dependencies"]
        if toolchain_info.report_unused_deps
        else [],
        ...
        cmd_args(
            ...
            "/=",
            cmd_args(ctx.label.path).replace_regex("\\\\", "/")
            if exec_is_windows
            else ctx.label.path,
            "/",
            delimiter="",
    ...
    if crate_type_linked(params.crate_type) and not common_args.is_check:
        ...
        if rust_cxx_link_group_info:
            inherited_non_rust_link_args = LinkArgs(
                infos=rust_cxx_link_group_info.filtered_links
                + [rust_cxx_link_group_info.symbol_files_info],
            )
    ...
    (diag, build_status) = _rustc_invoke(
        ...
        only_artifact="metadata"
        if toolchain_info.pipelined and emit == Emit("metadata")
        else None,
    )
    ...
    if common_args.is_check:
        ...
        (clippy_diag, _) = _rustc_invoke(
            ...
            # Lints go first to allow other args to override them.
            rustc_cmd=cmd_args(
                compile_ctx.clippy_wrapper, clippy_lints, rustc_cmd, clippy_emit_args
            ),
            env=clippy_env,
            diag="clippy",
    ...
    if is_binary and dwp_available(compile_ctx.cxx_toolchain_info):
        dwp_output = dwp(
            ...
            compile_ctx.cxx_toolchain_info,
            emit_output,
            identifier="{}/__{}_{}_dwp".format(
                common_args.subdir, common_args.tempfile, str(emit)
            ),
...
def _dependency_args(
    ...
    for dep in resolve_rust_deps(ctx, include_doc_deps=is_rustdoc_test):
        ...
        use_rmeta = is_check or (
            compile_ctx.toolchain_info.pipelined
            and not crate_type_codegen(crate_type)
            and not is_rustdoc_test
        )
...
def simple_symlinked_dirs(
    ctx: AnalysisContext, prefix: str, artifacts: dict[Artifact, None]
) -> cmd_args:
...
def dynamic_symlinked_dirs(
    ...
    compile_ctx: CompileContext,
    prefix: str,
    artifacts: dict[Artifact, CrateName],
) -> cmd_args:
    ...
    for artifact, crate in artifacts.items():
        relative_path = cmd_args(artifact).relative_to(
            transitive_dependency_dir.project("i")
        )
    ...
    return cmd_args(transitive_dependency_dir, format="@{}/dirs").hidden(
        artifacts.keys()
    )


def _lintify(
    flag: str, clippy: bool, lints: list[ResolvedStringWithMacros]
) -> cmd_args:
    return cmd_args(
...
def _rustc_flags(
    flags: list[[str, ResolvedStringWithMacros]]
) -> list[[str, ResolvedStringWithMacros]]:
...
def _compute_common_args(
    ...
    subdir = "{}-{}-{}-{}".format(
        crate_type.value, params.reloc_model.value, dep_link_style.value, emit.value
    )
    if is_rustdoc_test:
    ...
    edition = (
        ctx.attrs.edition
        or toolchain_info.default_edition
        or fail(
            "missing 'edition' attribute, and there is no 'default_edition' set by the toolchain"
        )
    )

    crate = attr_crate(ctx)
    if crate.dynamic:
        crate_name_arg = cmd_args(
            "--crate-name", cmd_args("@", crate.dynamic, delimiter="")
        )
    else:
    ...
    args = cmd_args(
        ...
        if not is_rustdoc_test
        else [],
        ["-Cprefer-dynamic=yes"] if crate_type == CrateType("dylib") else [],
        ["--target={}".format(toolchain_info.rustc_target_triple)]
    ...
    common_args = CommonArgsInfo(
        ...
        tempfile=tempfile,
        short_cmd="{},{},{}".format(
            crate_type.value, params.reloc_model.value, emit.value
        ),
        is_check=is_check,
        crate_map=crate_map,
    )
...
def _clippy_wrapper(
    ctx: AnalysisContext, toolchain_info: RustToolchainInfo
) -> cmd_args:
    clippy_driver = cmd_args(toolchain_info.clippy_driver)
    rustc_print_sysroot = cmd_args(
        toolchain_info.compiler, "--print=sysroot", delimiter=" "
    )
    if toolchain_info.rustc_target_triple:
        rustc_print_sysroot.add(
            "--target={}".format(toolchain_info.rustc_target_triple)
        )
    ...
    if ctx.attrs._exec_os_type[OsLookup].platform == "windows":
        wrapper_file, _ = ctx.actions.write(
            ...
            [
                ...
                cmd_args(
                    rustc_print_sysroot,
                    format='FOR /F "tokens=* USEBACKQ" %%F IN (`{}`) DO (',
                ),
                "set SYSROOT=%%F",
                ")",
...
def _crate_root(ctx: AnalysisContext, srcs: list[str], default_roots: list[str]) -> str:
    candidates = set()
    ...
    fail(
        "Could not infer crate_root. candidates=%s\nAdd 'crate_root = \"src/example.rs\"' to your attributes to disambiguate."
        % candidates.list()
    )
...
def _rustc_emit(
    ...
    subdir: str,
    params: BuildParams,
) -> (Artifact, cmd_args, [Artifact, None]):
    toolchain_info = compile_ctx.toolchain_info
    ...
    pipeline_artifact = (
        ...
        and emit in (Emit("metadata"), Emit("link"))
        and not crate_type_codegen(crate_type)
    )
    ...
    if emit in predeclared_outputs:
        ...
        filename = (
            subdir + "/" + output_filename(simple_crate, emit, params, extra_hash)
        )
    ...
    if pipeline_artifact:
        ...
        emit_args.add(
            cmd_args(
                "--emit=metadata=", emit_output.as_output(), metaext, delimiter=""
            ),
        ...
        emit_args.add(
            cmd_args("--emit=", emit.value, "=", emit_output.as_output(), delimiter="")
        )
    ...
    if emit != Emit("expand"):
        ...
        if ctx.attrs.incremental_enabled:
            ...
            incremental_out = ctx.actions.declare_output(
                "{}/extras/incremental/{}".format(subdir, build_mode)
            )
            incremental_cmd = cmd_args(
                incremental_out.as_output(), format="-Cincremental={}"
            )
...
def _rustc_invoke(
    ...
    only_artifact: [None, str] = None,
) -> (dict[str, Artifact], [Artifact, None]):
    toolchain_info = compile_ctx.toolchain_info
    ...
    if toolchain_info.failure_filter:
        ...
        build_status = ctx.actions.declare_output(
            "{}_build_status-{}.json".format(prefix, diag)
        )
        compile_cmd.add(
            cmd_args(build_status.as_output(), format="--failure-filter={}")
        )
...
def _long_command(
    ctx: AnalysisContext, exe: RunInfo, args: cmd_args, argfile_name: str
) -> cmd_args:
...
def _process_env(
    compile_ctx: CompileContext, env: dict[str, [ResolvedStringWithMacros, Artifact]]
