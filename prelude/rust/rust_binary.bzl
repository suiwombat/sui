...
def _rust_binary_common(
...
) -> (list[[DefaultInfo, RunInfo]], cmd_args):
    ...
    specified_link_style = (
        LinkStyle(ctx.attrs.link_style)
        if ctx.attrs.link_style
        else DEFAULT_STATIC_LINK_STYLE
    )
    ...
    for link_style in LinkStyle:
        ...
        name = (
            link_style.value + "/" + output_filename(simple_crate, Emit("link"), params)
        )
        ...
        if enable_link_groups(ctx, link_style, specified_link_style, is_binary=True):
            ...
            link_group_preferred_linkage = (
                rust_cxx_link_group_info.link_group_preferred_linkage
            )
        ...
        if link_style == LinkStyle("shared") or rust_cxx_link_group_info != None:
            ...
            for soname, shared_lib in traverse_shared_library_info(shlib_info).items():
                ...
                if rust_cxx_link_group_info == None or is_link_group_shlib(
                    label, link_group_ctx
                ):
        ...
        sub_targets_for_link_style["shared-libraries"] = [
            DefaultInfo(
                    ...
                    name + ".shared-libraries.json",
                    {
                        "libraries": [
                            "{}:{}[shared-libraries][{}]".format(
                                ctx.label.path, ctx.label.name, name
                            )
                            for name in shared_libs.keys()
                        ],
                        "librariesdwp": [
                            "{}:{}[shared-libraries][{}][dwp]".format(
                                ctx.label.path, ctx.label.name, name
                            )
                        ...
                        "rpathtree": [
                            "{}:{}[rpath-tree]".format(ctx.label.path, ctx.label.name)
                        ]
                ...
                sub_targets={
                    name: [
                        DefaultInfo(
                            ...
                            sub_targets={"dwp": [DefaultInfo(default_output=lib.dwp)]}
                            if lib.dwp
                            else {},
                        )
                    ]
        ...
        if shared_libs_symlink_tree:
            sub_targets_for_link_style["rpath-tree"] = [
                DefaultInfo(
                    default_output=shared_libs_symlink_tree,
                    other_outputs=[lib.output for lib in shared_libs.values()]
                    + [lib.dwp for lib in shared_libs.values() if lib.dwp],
                )
            ]

        if rust_cxx_link_group_info:
            sub_targets_for_link_style[LINK_GROUP_MAP_DATABASE_SUB_TARGET] = [
                get_link_group_map_json(ctx, filtered_targets)
            ]
            readable_mappings = {}
            for node, group in link_group_mappings.items():
                readable_mappings[group] = readable_mappings.get(group, []) + [
                    "{}//{}:{}".format(node.cell, node.package, node.name)
                ]
            sub_targets_for_link_style[LINK_GROUP_MAPPINGS_SUB_TARGET] = [
                DefaultInfo(
                    default_output=ctx.actions.write_json(
    ...
    sub_targets.update(
        {k: [DefaultInfo(default_output=v)] for k, v in extra_compiled_targets}
    )
    ...
    if dupmbin_toolchain:
        sub_targets[DUMPBIN_SUB_TARGET] = get_dumpbin_providers(
            ctx, compiled_outputs.link, dupmbin_toolchain
        )
...
def rust_test_impl(
    ctx: AnalysisContext,
) -> list[[DefaultInfo, RunInfo, ExternalRunnerTestInfo]]:
    compile_ctx = compile_context(ctx)
    toolchain_info = compile_ctx.toolchain_info
